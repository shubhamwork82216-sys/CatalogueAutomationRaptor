...
import hashlib
import imagehash
# cv2, torch, pytesseract, transformers loaded lazily inside tabs (Streamlit Cloud RAM limit)
import cv2
# import easyocr  # Disabled for cloud deployment
import openpyxl
...
# ── selenium / undetected-chrome ── DISABLED for Render.com ──
...
import uuid
import zipfile
# pytesseract, torch, transformers imported lazily inside Image QC tab
# to avoid loading ~2GB into RAM on startup (Streamlit Cloud RAM limit)
try:
    import pytesseract
...
# =========================================================
# SELENIUM HELPER — DISABLED for Render.com
# Selenium/Chrome cannot run in cloud environments.
# get_image_with_selenium() now returns None gracefully.
# =========================================================
# _uc_driver = None  # Disabled

def _start_uc_driver():
    pass  # Disabled for Render.com — Chrome not available

def get_image_with_selenium(url):
    # Disabled for Render.com — returns None so callers skip gracefully
    return None
...
# =========================================================
        ...
        def analyze_resolution(url):
            try:
                ...
                if r.status_code == 200 and "image" in r.headers.get("Content-Type",""):
                    ...
                    # Selenium disabled on Render.com — skip blocked images
                    return url, None, None, None, None, "Blocked (403)"
                else:
                    return url, None, None, None, None, f"{r.status_code}"
            except:
                # Selenium disabled — return fetch error
                return url, None, None, None, None, "Fetch Error"
        ...
        def get_image_bytes(url):
            try:
                ...
                if r.status_code == 200 and "image" in r.headers.get("Content-Type",""):
                    ...
                    # Selenium disabled on Render.com
                    return None
            except:
                pass
            return None
    ...
    with tab3:
        ...
        st.warning(
            "⚠️ **Streamlit Cloud Note:** BLIP AI watermark check requires ~1GB RAM. "
            "If the app crashes here, disable the BLIP checkbox below and use CV-only mode. "
            "For full BLIP support, use Google Cloud Run deployment."
        )
        ...
        def _load_blip():
            if not BLIP_AVAILABLE:
                st.error("BLIP model not available. Install transformers and torch.")
                return None, None
            from transformers import BlipProcessor, BlipForQuestionAnswering
    ...
    with tab4:
        ...
        st.warning(
            "⚠️ **Streamlit Cloud Note:** SRGAN requires PyTorch (~2GB RAM). "
            "Streamlit Community Cloud free tier has ~1GB RAM limit. "
            "This feature works best on Google Cloud Run or a paid Streamlit plan."
        )
