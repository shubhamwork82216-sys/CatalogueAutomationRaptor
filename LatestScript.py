...
# =========================================================
_uc_driver   = None
_uc_init_lock = threading.Lock()   # prevents multiple threads initialising the driver at once
_uc_call_lock = threading.Lock()   # serialises all selenium calls (WebDriver is not thread-safe)

def _start_uc_driver():
    """
    Initialise the undetected-chrome driver exactly once, even when called
    from multiple threads simultaneously.

    Also deletes the stale undetected_chromedriver.exe that causes
    WinError 183 ("cannot create a file when that file already exists")
    when the previous session was killed without a clean shutdown.
    """
    global _uc_driver
    if _uc_driver is not None:          # fast path — no lock needed
        return
    with _uc_init_lock:                 # only one thread enters the block
        if _uc_driver is not None:      # double-checked locking
            return
        # ── Remove stale chromedriver exe that causes WinError 183 ───────
        _stale = os.path.join(
        ...
        if os.path.exists(_stale):
            try:
                ...
                pass   # already gone or locked — UC will handle it
        # ── Start driver ─────────────────────────────────────────────────
...
def get_image_with_selenium(url):
    global _uc_driver
    # _uc_call_lock wraps BOTH init AND the actual driver calls.
    # WebDriver is not thread-safe — serialise everything through one lock.
    with _uc_call_lock:
        _start_uc_driver()
        try:
            _uc_driver.get(url)
...
if tool == "📦 Catalogue Automation":
    ...
    def safe_flat_dict(val):
        """
        Parse val into a flat  key → value  dict, handling any scraper format:

          Format 1 — flat dict (standard):
              {'UPC': '812958004937', 'Color': 'Black'}

          Format 2 — list of single-key dicts (Grainger-style):
              [{'Model No': 'RC5510-S'}, {'UPC': '812958004937'}, {'Color': 'Black'}]

          Format 3 — list of key/value pair dicts:
              [{'key': 'UPC', 'value': '812958004937'}, {'key': 'Color', 'value': 'Black'}]
              also handles 'name'/'attribute' instead of 'key'

          Format 4 — list of [key, value] two-element lists:
              [['UPC', '812958004937'], ['Color', 'Black']]

          Format 5 — multi-key dict inside a list:
              [{'UPC': '812958004937', 'Color': 'Black', 'Length': '9.5'}]
        ...
        if isinstance(val, dict):
            return val                    # already a flat dict
        ...
        # Already a dict after parsing
        if isinstance(parsed, dict):
            return parsed

        # List — normalise every element into a flat dict
        if isinstance(parsed, list):
            flat = {}
            # Keys that signal "this is a group name, not a real attribute"
            _GROUP_KEYS  = {"heading", "group", "section", "category", "tab",
                            "label", "title", "name", "type"}
            # Keys that hold the actual nested attribute dict
            _DATA_KEYS   = {"data", "attributes", "specs", "specifications",
                            "properties", "values", "items", "fields"}

            for item in parsed:
                if isinstance(item, dict):
                    keys_lower = {str(k).lower(): k for k in item}

                    # Format 6 — grouped/sectioned:
                    # {'heading': 'General', 'data': {'UPC': '...', 'Color': '...'}}
                    data_key = next(
                        (orig for lo, orig in keys_lower.items() if lo in _DATA_KEYS), None
                    )
                    if data_key is not None and isinstance(item[data_key], dict):
                        flat.update(item[data_key])
                        continue

                    # Format 3 — explicit key/value structure:
                    # {'key': 'UPC', 'value': '...'} or {'name': 'UPC', 'value': '...'}
                    if "value" in keys_lower:
                        kv_key = next(
                            (orig for lo, orig in keys_lower.items()
                             if lo in ("key", "name", "attribute")), None
                        )
                        if kv_key is not None:
                            flat[str(item[kv_key])] = item[keys_lower["value"]]
                            continue

                    # Format 2 (single-key) or Format 5 (multi-key) — merge all
                    flat.update(item)

                elif isinstance(item, (list, tuple)) and len(item) == 2:
                    # Format 4: [key, value] pair
    ...
    def extract_spec_fast(row):
        ...
        for col in ["Specification 1", "Specification 2", "Miscellaneous"]:
            # safe_flat_dict handles all scraper formats:
            # flat dict, list-of-single-key-dicts, key/value pair lists, etc.
            spec_combined.update(safe_flat_dict(row.get(col, "")))
        out = {"l": [], "w": [], "h": [], "upc": "", "wt": []}
    ...
    def crossref_match(live_row, scrape_df,
                       _mpn_idx=None, _cross_idx=None):
        """
        O(1) cross-reference lookup using pre-built index dicts.
        Falls back to O(n) scan only when index dicts are not supplied
        (backward-compatibility shim — never called that way in Step-1).
        """
        live_mpn   = str(live_row.get("Clean_MPN", "")).strip()
        live_cross = str(live_row.get("cross_ref", "")).strip().lower()
        if not live_cross:
            return None
        if _mpn_idx is not None:
            # live's cross_ref value matches a scrape MPN
            row = _mpn_idx.get(live_cross)
            if row is not None:
                return row
        if _cross_idx is not None:
            # a scrape row's cross_ref value matches live's MPN
            row = _cross_idx.get(live_mpn)
            if row is not None:
                return row
        if _mpn_idx is None and _cross_idx is None:
            # legacy fallback (not used in Step-1 anymore)
    ...
    def _normalize_scrape_cols(df):
        """
        Rename scrape DataFrame columns to the canonical names the script
        expects, regardless of whether the scraper used Title Case + spaces
        (standard) or lowercase + underscores (Hubbell/alternate scrapers).

        e.g.  'product_description_1' → 'Product Description 1'
        ...
        _alias = {
            ...
            # navigation / url
            "breadscrumbs":           "Breadscrumbs",
            "breadcrumbs":            "Breadscrumbs",
            "bread_crumbs":           "Breadscrumbs",
            "bread_scrumbs":          "Breadscrumbs",
            "url":                    "URL",
            # media
        ...
        for col in df.columns:
            canonical = _alias.get(col.strip().lower())
    ...
    if st.button("▶ Run Step-1 Mapping"):
        ...
        # ── Normalise scrape column names before any lookups ───────────────────
        # Handles scrapers that output lowercase+underscore names
        # e.g. 'product_description_1' → 'Product Description 1'
        scrape = _normalize_scrape_cols(scrape)
        ...
        # ── Build O(1) lookup indexes ONCE (fixes 14-hour issue) ──────────────
        # scrape_mpn_idx   : Clean_MPN → first matching scrape row
        # scrape_cross_idx : scrape cross_ref value → scrape row
        #                    (used when live's cross_ref matches a scrape MPN or vice-versa)
        scrape_mpn_idx   = {}
        scrape_cross_idx = {}
        for _, srow in scrape.iterrows():
            mpn = srow["Clean_MPN"]
            if mpn and mpn not in scrape_mpn_idx:
                scrape_mpn_idx[mpn] = srow
            cr = str(srow.get("cross_ref", "")).strip().lower()
            if cr and cr not in scrape_cross_idx:
                scrape_cross_idx[cr] = srow
        step1_status.info(
            f"⏳ Index built — {len(scrape_mpn_idx)} unique scrape MPNs. "
            f"Mapping {len(live)} live rows…"
        )
        ...
        for i, row in live.iterrows():
            # ── Tier 1: O(1) direct MPN match ─────────────────────────────────
            matched_scrape_row = scrape_mpn_idx.get(row["Clean_MPN"])
            if matched_scrape_row is not None:
                existing_df.at[i, "Remark_1"] = "mapped"
            else:
                # ── Tier 2: O(1) cross-reference match ────────────────────────
                matched_scrape_row = crossref_match(
                    row, scrape,
                    _mpn_idx=scrape_mpn_idx,
                    _cross_idx=scrape_cross_idx,
                )
                if matched_scrape_row is not None:
        ...
        for idx in range(total_ob):
            ...
            # Image Urls — column already normalised by _normalize_scrape_cols
            ...
            # Breadscrumbs / URL — normalised column names used here
...
{retry_block}"""
    ...
    def _tc_build_output_excel(df_existing_full, df_existing_filtered, existing_orig_idx,
        ...
        for label, df in [("Existing", df_existing_full), ("New_Onboarding", df_new_full)]:
            ...
            # Build desired column order — new output columns inserted at the right positions.
            # Deduplicate while preserving order so that uploading a Step-2 file (which
            # already has standardized_item_name, generated_title, etc.) does NOT create
            # duplicate columns that break .at[] access in Item Name Only mode.
            _desired = (
                cols[:insert_pos]
        ...
        def _process_sheet(label, df_filtered, orig_idx, column_map):
            ...
            # Skipped when run_mode == "item_name"
            # ══════════════════════════════════════════════════════════════
            if run_mode in ("both", "title"):
                n_batches = (total + batch_size - 1) // batch_size
                log_callback(f"[{label}]  {total} rows · {n_batches} batches (title generation)", "accent")
                ...
                # Item Name Only — titles already exist in the uploaded file;
                # just advance the progress counter so the bar moves correctly.
                log_callback(
                    f"[{label}] ⏩ Title generation skipped (Item Name Only mode) — "
                    f"using existing standardized_item_name & generated_title columns", "info"
                )
                processed[0] += total
                progress_callback(processed[0] / max(total_rows, 1))

            # ══════════════════════════════════════════════════════════════
            # PHASE 2 — ITEM NAME MATCHING
            # Skipped when run_mode == "title"
            # ══════════════════════════════════════════════════════════════
            if run_mode == "title":
                log_callback(f"[{label}] ⏩ Item name matching skipped (Title Only mode)", "info")
                return

            if not reference_names:
                log_callback(f"[{label}] ⚠ No reference list uploaded — item name matching skipped", "warn")
                for _idx in orig_idx:
            ...
            for _idx in orig_idx:
                ...
                if not nm or nm.lower() in ("nan", "none", "") or nm.startswith("ERROR:"):
                    out_df.at[_idx, "fuzzy_matched_item_name"] = ""
                    out_df.at[_idx, "fuzzy_score"]             = None
            ...
            if unique_ai_ctx:
                log_callback(
                    f"[{label}] AI context match: {len(unique_ai_ctx)} unique names "
                    f"covering {sum(len(v) for v in name_to_rows.values())} rows "
                    f"(top-30 candidates each)…", "info"
                )
    ...
    with s2_col_right:
        # ── Run mode ──────────────────────────────────────────────────────
        s2_run_mode = st.radio(
            "⚙️ What to run",
            ["Both (Title + Item Name)", "Title Only", "Item Name Only"],
            index=0,
            horizontal=True,
            key="s2_run_mode",
            help=(
                "**Both** — generate titles then match item names (default).\n\n"
                "**Title Only** — generate titles, skip item name matching.\n\n"
                "**Item Name Only** — skip title generation; the uploaded file must already "
                "contain `standardized_item_name` and `generated_title` columns."
            ),
        )
        _s2_run_mode_key = {
            "Both (Title + Item Name)": "both",
            "Title Only":               "title",
            "Item Name Only":           "item_name",
        }[s2_run_mode]
    ...
    # For "Item Name Only": API key is needed only when a reference file is
    # also uploaded (Tier-3 AI item matching will use the API).
    # For "Title Only" / "Both": API key always needed if model is cloud-based.
    _key_required = _need_key and (
        _s2_run_mode_key != "item_name" or bool(reference_names)
    )
    can_run_s2 = (
        step2_file is not None
        and (bool(s2_api_key) if _key_required else True)
        and bool(s2_scope)
    ...
    if st.session_state.s2_running:
        ...
        try:
            ...
            # ── Item Name Only: validate required columns exist ───────────
            if _s2_run_mode_key == "item_name":
                _required_cols = {"standardized_item_name", "generated_title"}
                _s1_missing = _required_cols - set(df_existing.columns)
                _s2_missing = _required_cols - set(df_new.columns)
                if _s1_missing or _s2_missing:
                    _tc_log(
                        "❌ 'Item Name Only' mode requires the uploaded file to already "
                        "contain 'standardized_item_name' and 'generated_title' columns "
                        "(i.e. upload a Step-2 output, not a Step-1 output). "
                        f"Sheet1 missing: {_s1_missing or '—'} | "
                        f"Sheet2 missing: {_s2_missing or '—'}",
                        "err"
                    )
                    _render_logs()
                    ...
                    _tc_log(
                        "✅ Item Name Only mode — standardized_item_name & generated_title "
                        "columns found. Title generation will be skipped.", "ok"
                    )
                    _render_logs()
...
# =========================================================
    ...
    with tab1:
        ...
        # ── Sheet selector (shown once file is uploaded) ──
        r1_selected_sheets = []
        if file1:
            file1.seek(0)
            _r1_xl        = pd.ExcelFile(file1)
            _r1_all_sheets = _r1_xl.sheet_names
            r1_selected_sheets = st.multiselect(
                "📋 Select sheets to run resolution check on",
                options=_r1_all_sheets,
                default=_r1_all_sheets,
                key="r1_sheet_sel",
                help="Choose Existing, New_Onboarding, or any combination of sheets",
            )
        ...
        _can_run_r1 = file1 is not None and len(r1_selected_sheets) > 0
        if st.button("Run Resolution Check", key="r1_btn", disabled=not _can_run_r1):
            file1.seek(0)
            xl            = pd.ExcelFile(file1)
            output_buffer = BytesIO()
            sheets_to_run = r1_selected_sheets   # only user-selected sheets
            total_sheets  = len(sheets_to_run)
            sheet_prog    = st.progress(0)
            sheet_status  = st.empty()
            global_start  = time.time()

            with pd.ExcelWriter(output_buffer, engine="openpyxl") as writer:
                for s_idx, sheet in enumerate(sheets_to_run):
                    df = xl.parse(sheet)
                    ...
                    sheet_status.info(f"✅ Sheet {s_idx+1}/{total_sheets} done: {sheet}")
